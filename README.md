...
- [ ] PDF 下载：不做 weasyprint（依赖重），前端 `window.print()` 直接打印模态框 A4 样式，零依赖
- [ ] 同票多商品项：支持一条申报单包含多个商品（表格式输入，每行一个商品）
- [ ] 导入真实数据：分阶段 — ① 美国 HTS（最大贸易伙伴） ② OFAC 制裁清单 ③ 中国税则更新
- [ ] HS 归类页关键条文高亮（当前推理路径已展示，可加 WCO 注释原文引用样式）
- [ ] 风险看板空状态增加示例数据切换开关
