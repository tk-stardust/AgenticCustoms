...
7. 目前系统仅支持美国（US）、欧盟（EU）、越南（VN）三个目标国/地区的关税税率和合规数据。如果用户问及其他国家（如日本、韩国等），主动告知当前支持范围，并询问是否需要换成这三个地区之一
"""
...
def clear_agent_memory(session_id: str):
    """清除指定会话的 Agent 记忆（MemorySaver 中的对话历史）"""
    try:
        for attr in ("_checkpoints", "storage", "_saved"):
            store = getattr(_memory, attr, None)
        ...
        for attr in ("_writes",):
            ...
            if store is not None:
                store.pop(session_id, None)
        logger.info("chat.memory.cleared", session_id=session_id)
    except Exception as e:
        logger.warning("chat.memory.clear.failed", session_id=session_id, error=str(e))
